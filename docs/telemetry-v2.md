...
`product` содержит только базовые metadata и закрытое product-specific значение `kind`. `outcome` передаёт завершённое безопасное действие через закрытые enums `result`, `source`, `mode`, `count_bucket`, `duration_bucket` и `failure_code`; поля, не разрешённые конкретному product/kind, отклоняются. Произвольные `payload`, `attributes`, текстовые значения, URL, пути и пользовательские идентификаторы отсутствуют в контракте и отклоняются strict decoder.
